const fs = require('fs');
let content = fs.readFileSync('src/config/data.ts', 'utf8');

// Strip all lat/lng lines
content = content.replace(/[ \t]*lat: [0-9.]+,\n/g, '');
content = content.replace(/[ \t]*lng: [0-9.]+,?\n/g, '');
...
// Now safely inject exactly once per title using an exact title match.
for (const [title, [lat, lng]] of Object.entries(coords)) {
  const safeTitle = title.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const regex = new RegExp(`(title: "${safeTitle}",[\\s\\S]*?mapsUrl: "[^"]*")`);
  content = content.replace(regex, `$1,\n    lat: ${lat},\n    lng: ${lng}`);
}

fs.writeFileSync('src/config/data.ts', content, 'utf8');
console.log('Fixed coordinates successfully.');
